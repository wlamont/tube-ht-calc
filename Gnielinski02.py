...
def calc(entries):
   ...
   print 'f=',f
   Nu = (f/8)*(Re-1000)*Pr/(1+12.7*pow(f/8,0.5)*(pow(Pr,2.0/3.0)-1))
   print 'Nu=',Nu
...
def aboutDB():
   ...
   root = Tk()
   #print units.get()
   lab1 = Label(root, text = 'Nu_d = (f/8)(Re-1000)*Pr/(1 + 12.7(f/8)^0.5(Pr^{2/3}-1))')
   lab1.pack(side = TOP)
   lab2 = Label(root, text = '3000 < Re <5x10^6 and 0.5 < Pr < 2000, ~10% Error')
   lab2.pack(side = TOP)
